...
from neuronlp2.models import DeepBiAffine, NeuroMST, StackPtrNet, DeepBiAffineEnsemble, StackPtrNetEnsemble
...
def score(args):
    ...
    for model_path in args.model_path.split(":"):
        network, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, punct_set, alg, hyps = load_model(logger, model_path, args)
        networks.append(network)
    
    if alg == 'graph':
        network = DeepBiAffineEnsemble(networks)
    else:
        network = StackPtrNetEnsemble(networks)
    ...
    network.eval()
    with torch.no_grad(), open(args.result_path+".score", "w") as fo:
        print('Scoring...')
        start_time = time.time()
        for data in iterate_data(data_test, args.batch_size):
            ...
            if alg == 'graph':
                ...
                loss_arc, loss_type = network.score(words, chars, postags, heads, types, mask=masks)
            else:
                ...
                loss_arc, loss_type = network.score(words, chars, postags, heads, stacked_heads, children, siblings, stacked_types, mask_e=masks_enc, mask_d=masks_dec)
            for x in (loss_arc + loss_type).tolist():
                fo.write(f"{x:.6f}\n")
        print('Time: %.2fs' % (time.time() - start_time))
                
def eval(alg, data, network, pred_writer, gold_writer, punct_set, word_alphabet, pos_alphabet, device, beam=1, batch_size=256, kbest=1):
    accum_ucorr = 0.0
    accum_lcorr = 0.0
    accum_total = 0
    accum_ucomlpete = 0.0
    ...
    for data in iterate_data(data, batch_size):
        ...
        if alg == 'graph':
            masks = data['MASK'].to(device)
            heads_pred, types_pred = network.decode(words, chars, postags, mask=masks, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS, kbest=kbest)
        else:
            masks = data['MASK_ENC'].to(device)
            heads_pred, types_pred = network.decode(words, chars, postags, mask=masks, beam=beam, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS, kbest=kbest)

        words = words.cpu().numpy()
        postags = postags.cpu().numpy()
        
        bsz = heads_pred.shape[0]
        for i in range(bsz):
            for k in range(kbest):
                L = lengths[i]
                pred_writer.write(words[i:i+1], postags[i:i+1], np.clip(heads_pred[i:i+1,k,:], a_min=0, a_max=L-1), types_pred[i:i+1,k,:], lengths[i:i+1], symbolic_root=True)
                gold_writer.write(words[i:i+1], postags[i:i+1], heads[i:i+1], types[i:i+1], lengths[i:i+1], symbolic_root=True)

        heads_pred = heads_pred[:,0,:]
        types_pred = types_pred[:,0,:]
...
def load_model(logger, model_path, args):
    device = torch.device('cuda', 0) if args.cuda else torch.device('cpu')
    ...
    arc_space = hyps['arc_space']
    type_space = hyps['type_space']
    p_in = args.p_in if args.p_in is not None else hyps['p_in']
    p_out = args.p_out if args.p_out is not None else hyps['p_out']
    p_rnn = args.p_rnn if args.p_rnn is not None else hyps['p_rnn']
    activation = hyps['activation']
    prior_order = None
    ...
    model = "{}-{}".format(model_type, mode)
    logger.info("Network: %s, num_layer=%s, hidden=%d, act=%s" % (model, num_layers, hidden_size, activation))
    return network, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, punct_set, alg, hyps
...
def parse(args):
    ...
    pred_writer = CoNLLXWriter(word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
    gold_writer = CoNLLXWriter(word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
    pred_filename = args.result_path + '.pred'
    pred_writer.start(pred_filename)
    gold_filename = args.result_path + '.gold'
    gold_writer.start(gold_filename)

    if args.p_rnn is None and args.p_in is None and args.p_in is None:
        network.eval()
    with torch.no_grad():
        print('Parsing...')
        start_time = time.time()
        eval(alg, data_test, network, pred_writer, gold_writer, punct_set, word_alphabet, pos_alphabet, device, beam, batch_size=args.batch_size, kbest=args.kbest)
        print('Time: %.2fs' % (time.time() - start_time))
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--mode', choices=['train', 'parse', 'score_reference'], required=True, help='processing mode')
    ...
    args_parser.add_argument('--result_path', help='path to ouput', default=None)
    args_parser.add_argument('--seed', help="random seed", type=int, default=123)
    args_parser.add_argument('--kbest', help="kbest", type=int, default=1)
    args_parser.add_argument('--p_in', help="dropout", type=float, default=None)
    args_parser.add_argument('--p_out', help="dropout", type=float, default=None)
    args_parser.add_argument('--p_rnn', help="dropout", type=float, default=None)
 
    args = args_parser.parse_args()
    if args.p_rnn is not None:
        args.p_rnn = (args.p_rnn, args.p_rnn)
    ...
    if args.mode == 'train':
        ...
        parse(args)
    elif args.mode == 'score_reference':
        score(args)
