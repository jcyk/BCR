...
class DeepBiAffine(nn.Module):
    ...
    def decode(self, input_word, input_char, input_pos, mask=None, leading_symbolic=0, return_energy=False, kbest=1):
        """
        Args:
            input_word: Tensor
        ...
        return parser.decode_MST(energy.cpu().numpy(), length.long().cpu().numpy(), leading_symbolic=leading_symbolic, labeled=True, kbest=kbest)

class DeepBiAffineEnsemble:
    ...
    def score(self, input_word, input_char, input_pos, heads, types, mask=None):
        num_parsers = len(self.parsers)
        all_arc_loss, all_type_loss = [], []
        for x in self.parsers:
            arc_loss, type_loss = x.loss(input_word, input_char, input_pos, heads, types, mask)
            all_arc_loss.append(arc_loss)
            all_type_loss.append(type_loss)
        avg_arc_loss = torch.stack(all_arc_loss).mean(0)
        avg_type_loss = torch.stack(all_type_loss).mean(0)
        return avg_arc_loss, avg_type_loss
    
    def decode(self, input_word, input_char, input_pos, mask=None, leading_symbolic=0, kbest=1):
        num_parsers = len(self.parsers)
        all_energy, all_length = [], []
        for x in self.parsers:
            energy, length = x.decode(input_word, input_char, input_pos, mask, leading_symbolic, return_energy=True)
            all_energy.append(energy)
            all_length.append(length)
        energy_avg = torch.stack(all_energy).mean(0)
        length_avg = torch.stack(all_length).mean(0)
    
        return parser.decode_MST(energy_avg.cpu().numpy(), length_avg.long().cpu().numpy(), leading_symbolic=leading_symbolic, labeled=True, kbest=kbest)
...
class StackPtrNet(nn.Module):
    ...
    def decode(self, input_word, input_char, input_pos, mask=None, beam=1, leading_symbolic=0, kbest=1):
        # reset noise for decoder
        batch = input_word.size(0)
        self.decoder.reset_noise(batch)
        ...
        heads = heads[:, :kbest].cpu().numpy()
        types = types[:, :kbest].cpu().numpy()
        return heads, types
    
    def _decode_prepare(self, input_word, input_char, input_pos, mask, beam):
        # reset noise for decoder
        batch = input_word.size(0)
        self.decoder.reset_noise(batch)
    ...
    def _decode_step_arc(self, t, prepared, curr_heads, curr_gpars, curr_sibs, mask_hyp, mask, steps):
        
        batch, num_hyp = curr_heads.shape
        enc_dim = prepared["output_enc"].shape[-1]
        # [batch, num_hyp, enc_dim]
        src_encoding = prepared["output_enc"].gather(dim=1, index=curr_heads.unsqueeze(2).expand(batch, num_hyp, enc_dim))

        if self.sibling:
            mask_sib = curr_sibs.gt(0).float().unsqueeze(2)
            output_enc_sibling = prepared["output_enc"].gather(dim=1, index=curr_sibs.unsqueeze(2).expand(batch, num_hyp, enc_dim)) * mask_sib
            src_encoding = src_encoding + output_enc_sibling

        if self.grandPar:
            output_enc_gpar = prepared["output_enc"].gather(dim=1, index=curr_gpars.unsqueeze(2).expand(batch, num_hyp, enc_dim))
            src_encoding = src_encoding + output_enc_gpar
        ...
        # output [batch * num_hyp, dec_dim]
        # hx [decoder_layer, batch * num_hyp, dec_dim]
        output_dec, hx = self.decoder.step(src_encoding.view(batch * num_hyp, -1), hx=prepared["hx"])
        prepared["hx"] = hx
        dec_dim = output_dec.size(1)
        # [batch, num_hyp, dec_dim]
        output_dec = output_dec.view(batch, num_hyp, dec_dim)
        ...
        # [batch, num_hyp, type_space]
        type_h = self.activation(self.type_h(output_dec))
        prepared["type_h"] = type_h
        # [batch, num_hyp, length]
        out_arc = self.biaffine(arc_h, prepared["arc_c"], mask_query=mask_hyp, mask_key=mask)
        # mask invalid position to -inf for log_softmax
        if mask is not None:
            minus_mask_enc = mask.eq(0).unsqueeze(1)
            out_arc.masked_fill_(minus_mask_enc, float('-inf'))
        ...
        return hyp_scores
    def _decode_step_type(self, prepared, batch, num_hyp, base_index, child_index):
        type_space = prepared["type_c"].size(2)
        # [batch, num_hyp, type_space]
        base_index_expand = base_index.unsqueeze(2).expand(batch, num_hyp, type_space)
        child_index_expand = child_index.unsqueeze(2).expand(batch, num_hyp, type_space)
        # [batch, num_hyp, num_labels]
        out_type = self.bilinear(prepared["type_h"].gather(dim=1, index=base_index_expand), prepared["type_c"].gather(dim=1, index=child_index_expand))
        hyp_type_scores = F.log_softmax(out_type, dim=2)
        return hyp_type_scores

class StackPtrNetEnsemble:
    def __init__(self, parsers):
        self.parsers = parsers
    ...
    def score(self, input_word, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, mask_e=None, mask_d=None):
        num_parsers = len(self.parsers)
        all_arc_loss, all_type_loss = [], []
        
        for x in self.parsers:
            arc_loss, type_loss = x.loss(input_word, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, mask_e, mask_d)
            all_arc_loss.append(arc_loss)
            all_type_loss.append(type_loss)
        avg_arc_loss = torch.stack(all_arc_loss).mean(0)
        avg_type_loss = torch.stack(all_type_loss).mean(0)
    ...
    def decode(self, input_word, input_char, input_pos, mask=None,  beam=1, leading_symbolic=0, kbest=1):
        #return self.parsers[0].decode(input_word, input_char, input_pos, mask,  beam, leading_symbolic)
        
        prepareds = []
        for x in self.parsers:
            prepared = x._decode_prepare(input_word, input_char, input_pos, mask, beam)
            prepareds.append(prepared)
        device = prepared['output_enc'].device
        batch, max_len, _ = prepared['output_enc'].size()
        ...
        num_steps = 2 * max_len - 1
        stacked_heads = torch.zeros(batch, 1, num_steps + 1, device=device, dtype=torch.int64)
        siblings = torch.zeros(batch, 1, num_steps + 1, device=device, dtype=torch.int64) if x.sibling else None
        hypothesis_scores = prepared['output_enc'].new_zeros((batch, 1))
        ...
        for t in range(num_steps):
            ...
            curr_gpars = heads.gather(dim=2, index=curr_heads.unsqueeze(2)).squeeze(2)
            all_hyp_scores = []
            for x, prepared in zip(self.parsers, prepareds):
                curr_sibs = siblings[:, :, t] if x.sibling else None
                hyp_scores = x._decode_step_arc(t, prepared, curr_heads, curr_gpars, curr_sibs, mask_hyp, mask, steps)
                all_hyp_scores.append(hyp_scores)
            
            hyp_scores = torch.stack(all_hyp_scores).mean(0)
            ...
            all_hyp_type_scores = []
            for x, prepared in zip(self.parsers, prepareds):
                hyp_type_scores = x._decode_step_type(prepared, batch, num_hyp, base_index, child_index)
                all_hyp_type_scores.append(hyp_type_scores)
            hyp_type_scores = torch.stack(all_hyp_type_scores).mean(0)
