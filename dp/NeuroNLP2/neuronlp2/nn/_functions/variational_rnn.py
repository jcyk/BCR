...
def VarFastLSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, noise_in=None, noise_hidden=None):
    #noise_in: (batch_size, self.input_size)
    #noise_hidden: (batch_size, self.hidden_size)
    if noise_in is not None:
        bsz, dim = noise_in.size()
        input = input.view(bsz, -1, dim) * noise_in.view(bsz, 1, dim)
        input = input.view(-1, dim)

    hx, cx = hidden
    if noise_hidden is not None:
        bsz, dim = noise_hidden.size()
        hx = hx.view(bsz, -1, dim) * noise_hidden.view(bsz, 1, dim)
        hx = hx.view(-1, dim)
