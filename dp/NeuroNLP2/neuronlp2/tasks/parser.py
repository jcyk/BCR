...
def decode_MST(energies, lengths, leading_symbolic=0, labeled=True, kbest=1):
    """
    ...
    pars = np.zeros([batch_size, kbest, max_length], dtype=np.int32)
    types = np.zeros([batch_size, kbest, max_length], dtype=np.int32) if labeled else None
    for i in range(batch_size):
        energy = energies[i]
...
#         chuLiuEdmonds()
        ...
        G = Graph.build(-orig_score_matrix)
        ...
        for tree in KBest(G).kbest():
            final_edges = dict()
            for tgt, src in enumerate(tree.to_array()):
                final_edges[tgt] = src
