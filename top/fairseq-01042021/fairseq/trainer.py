...
class Trainer(object):
    ...
    def valid_step(self, sample, raise_oom=False, subset=None):
            ...
            try:
                if subset is not None:
                    _loss, sample_size, logging_output = self.task.valid_step(
                        sample, self.model, self.criterion, subset=subset
                    )
                else:
                    _loss, sample_size, logging_output = self.task.valid_step(
        ...
        logging_output = self._reduce_and_log_stats(logging_outputs, sample_size, subset=subset)
    ...
    def _reduce_and_log_stats(self, logging_outputs, sample_size, grad_norm=None, subset=None):
        ...
        with metrics.aggregate() as agg:
            if logging_outputs is not None:
                if subset is not None:
                    self.task.reduce_metrics(logging_outputs, self.get_criterion(), subset=subset)
                else:
