...
def validate_and_save(
    ...
    do_validate = (
        ...
        or (end_of_epoch and epoch_itr.epoch % cfg.dataset.validate_interval == 0 and epoch_itr.epoch > cfg.dataset.minimum_validate_interval)
        or num_updates >= max_update
        or (
    ...
    for subset in subsets:
        ...
        with metrics.aggregate(new_root=True) as agg:
            for sample in progress:
                if type(task) == tasks.enconly_conv_nlu.EnconlyConvNLU:
                    trainer.valid_step(sample, subset=subset)
                else:
                    trainer.valid_step(sample)

        # log validation stats
        if "valid_inference" in subsets:  # special case for encoder only model
            if subset == "valid_inference":
